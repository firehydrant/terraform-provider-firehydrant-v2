// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/firehydrant/terraform-provider-firehydrant/internal/sdk/models/shared"
	"net/http"
)

type ReorderPostMortemReasonsRequest struct {
	ReportID                 string                          `pathParam:"style=simple,explode=false,name=report_id"`
	ReorderPostMortemReasons shared.ReorderPostMortemReasons `request:"mediaType=application/json"`
}

func (o *ReorderPostMortemReasonsRequest) GetReportID() string {
	if o == nil {
		return ""
	}
	return o.ReportID
}

func (o *ReorderPostMortemReasonsRequest) GetReorderPostMortemReasons() shared.ReorderPostMortemReasons {
	if o == nil {
		return shared.ReorderPostMortemReasons{}
	}
	return o.ReorderPostMortemReasons
}

type ReorderPostMortemReasonsResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// Update the order of contributing factors in a retrospective report
	PostMortemsReason *shared.PostMortemsReason
}

func (o *ReorderPostMortemReasonsResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *ReorderPostMortemReasonsResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *ReorderPostMortemReasonsResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *ReorderPostMortemReasonsResponse) GetPostMortemsReason() *shared.PostMortemsReason {
	if o == nil {
		return nil
	}
	return o.PostMortemsReason
}
