// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/firehydrant/terraform-provider-firehydrant/internal/sdk/models/shared"
	"net/http"
)

type GetSignalsEmailTargetRequest struct {
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (o *GetSignalsEmailTargetRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

type GetSignalsEmailTargetResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// Get a Signals email target by ID
	SignalsAPIEmailTarget *shared.SignalsAPIEmailTarget
}

func (o *GetSignalsEmailTargetResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *GetSignalsEmailTargetResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *GetSignalsEmailTargetResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *GetSignalsEmailTargetResponse) GetSignalsAPIEmailTarget() *shared.SignalsAPIEmailTarget {
	if o == nil {
		return nil
	}
	return o.SignalsAPIEmailTarget
}
