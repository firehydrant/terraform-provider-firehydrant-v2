// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/firehydrant/terraform-provider-firehydrant/internal/sdk/models/shared"
	"net/http"
)

type DeleteV1LifecyclesMilestonesMilestoneIDRequest struct {
	MilestoneID string `pathParam:"style=simple,explode=false,name=milestone_id"`
}

func (o *DeleteV1LifecyclesMilestonesMilestoneIDRequest) GetMilestoneID() string {
	if o == nil {
		return ""
	}
	return o.MilestoneID
}

type DeleteV1LifecyclesMilestonesMilestoneIDResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// Delete a milestone
	LifecyclesPhaseEntity *shared.LifecyclesPhaseEntity
}

func (o *DeleteV1LifecyclesMilestonesMilestoneIDResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *DeleteV1LifecyclesMilestonesMilestoneIDResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *DeleteV1LifecyclesMilestonesMilestoneIDResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}

func (o *DeleteV1LifecyclesMilestonesMilestoneIDResponse) GetLifecyclesPhaseEntity() *shared.LifecyclesPhaseEntity {
	if o == nil {
		return nil
	}
	return o.LifecyclesPhaseEntity
}
