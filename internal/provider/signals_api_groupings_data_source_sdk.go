...
func (r *SignalsAPIGroupingsDataSourceModel) RefreshFromSharedSignalsAPIGroupingPaginated(ctx context.Context, resp *shared.SignalsAPIGroupingPaginated) diag.Diagnostics {
	var diags diag.Diagnostics

	if resp != nil {
		if resp.Data != nil {
			r.Data = []tfTypes.SignalsAPIGrouping{}
			if len(r.Data) > len(resp.Data) {
				r.Data = r.Data[:len(resp.Data)]
			}
			for dataCount, dataItem := range resp.Data {
				var data tfTypes.SignalsAPIGrouping
				if dataItem.Action == nil {
					data.Action = nil
				} else {
					data.Action = &tfTypes.NullableSignalsAPIGroupingAction{}
					if dataItem.Action.Fyi == nil {
						data.Action.Fyi = nil
					} else {
						data.Action.Fyi = &tfTypes.NullableSignalsAPIGroupingActionFyi{}
						if dataItem.Action.Fyi.SlackChannels != nil {
							data.Action.Fyi.SlackChannels = []tfTypes.IntegrationsSlackSlackChannel{}
				...
				if dataItem.Strategy == nil {
					...
					data.Strategy = &tfTypes.NullableSignalsAPIGroupingStrategy{}
					if dataItem.Strategy.Substring == nil {
						data.Strategy.Substring = nil
					} else {
						data.Strategy.Substring = &tfTypes.NullableSignalsAPIGroupingStrategySubstring{}
		...
		if resp.Pagination == nil {
			...
			r.Pagination = &tfTypes.NullablePagination{}
