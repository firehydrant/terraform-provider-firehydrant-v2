...
func (r *TicketingProjectFieldMapEntityDataSourceModel) ToOperationsGetTicketingFieldMapRequest(ctx context.Context) (*operations.GetTicketingFieldMapRequest, diag.Diagnostics) {
...
func (r *TicketingProjectFieldMapEntityDataSourceModel) RefreshFromSharedTicketingProjectFieldMapEntity(ctx context.Context, resp *shared.TicketingProjectFieldMapEntity) diag.Diagnostics {
	var diags diag.Diagnostics

	if resp != nil {
		if resp.Body != nil {
			r.Body = []tfTypes.TicketingProjectFieldMapBodyEntity{}
			if len(r.Body) > len(resp.Body) {
				r.Body = r.Body[:len(resp.Body)]
			}
			for bodyCount, bodyItem := range resp.Body {
				var body tfTypes.TicketingProjectFieldMapBodyEntity
				if bodyItem.Cases != nil {
					body.Cases = []tfTypes.TicketingProjectFieldMapCasesEntity{}
					for casesCount, casesItem := range bodyItem.Cases {
						var cases tfTypes.TicketingProjectFieldMapCasesEntity
						if casesItem.ExternalValue == nil {
							cases.ExternalValue = nil
						} else {
							cases.ExternalValue = &tfTypes.NullableTicketingProjectFieldMapExternalValueEntity{}
				...
				if bodyItem.Else == nil {
					...
					body.Else = &tfTypes.NullableTicketingProjectFieldMapCasesElseEntity{}
					if bodyItem.Else.ExternalValue == nil {
						body.Else.ExternalValue = nil
					} else {
						body.Else.ExternalValue = &tfTypes.NullableTicketingProjectFieldMapExternalValueEntity{}
				...
				if bodyItem.ExternalValue == nil {
					...
					body.ExternalValue = &tfTypes.NullableTicketingProjectFieldMapExternalValueEntity{}
