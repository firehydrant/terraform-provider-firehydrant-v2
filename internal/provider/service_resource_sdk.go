...
func (r *ServiceResourceModel) ToSharedPostV1Services() *shared.PostV1Services {
	var name string
	...
	serviceTier := new(shared.ServiceTier)
	if !r.ServiceTier.IsUnknown() && !r.ServiceTier.IsNull() {
		*serviceTier = shared.ServiceTier(r.ServiceTier.ValueInt64())
	} else {
		serviceTier = nil
	}
	var functionalities []shared.Functionalities = []shared.Functionalities{}
	for _, functionalitiesItem := range r.Functionalities {
		summary := new(string)
		if !functionalitiesItem.Summary.IsUnknown() && !functionalitiesItem.Summary.IsNull() {
			*summary = functionalitiesItem.Summary.ValueString()
		} else {
			summary = nil
		}
		id := new(string)
		if !functionalitiesItem.ID.IsUnknown() && !functionalitiesItem.ID.IsNull() {
			*id = functionalitiesItem.ID.ValueString()
		} else {
			id = nil
		}
		functionalities = append(functionalities, shared.Functionalities{
			Summary: summary,
			ID:      id,
		})
	}
	var links []shared.Links = []shared.Links{}
	for _, linksItem := range r.Links {
		...
		links = append(links, shared.Links{
			Name:    name1,
			HrefURL: hrefURL,
			IconURL: iconURL,
		})
	...
	var teams []shared.Teams = []shared.Teams{}
	for _, teamsItem := range r.Teams {
		var id2 string
		id2 = teamsItem.ID.ValueString()

		teams = append(teams, shared.Teams{
			ID: id2,
		})
	}
	alertOnAdd := new(bool)
	...
	var externalResources []shared.ExternalResources = []shared.ExternalResources{}
	for _, externalResourcesItem := range r.ExternalResources {
		...
		externalResources = append(externalResources, shared.ExternalResources{
			RemoteID:       remoteID,
			ConnectionType: connectionType,
		})
	}
	out := shared.PostV1Services{
		Name:                  name,
...
func (r *ServiceResourceModel) RefreshFromSharedServiceEntity(resp *shared.ServiceEntity) {
	if resp != nil {
		...
		for externalResourcesCount2, externalResourcesItem2 := range resp.ExternalResources {
			var externalResources5 tfTypes.ExternalResourceEntity1
		...
		r.Functionalities = []tfTypes.Functionalities{}
		...
		for functionalitiesCount1, functionalitiesItem1 := range resp.Functionalities {
			var functionalities3 tfTypes.Functionalities
		...
		for linksCount3, linksItem3 := range resp.Links {
			var links7 tfTypes.LinksEntity1
...
func (r *ServiceResourceModel) ToSharedPatchV1ServicesServiceID() *shared.PatchV1ServicesServiceID {
	alertOnAdd := new(bool)
	if !r.AlertOnAdd.IsUnknown() && !r.AlertOnAdd.IsNull() {
		*alertOnAdd = r.AlertOnAdd.ValueBool()
	} else {
	...
	var checklists []shared.Checklists = []shared.Checklists{}
	for _, checklistsItem := range r.Checklists {
		var id string
		id = checklistsItem.ID.ValueString()

		// Service#update.checklists.removeService#update.checklists.remove impedance mismatch: boolean != stringtrace=["Service#create.resp.checklists.[].created_at"]
		var remove *bool
		checklists = append(checklists, shared.Checklists{
			ID:     id,
			Remove: remove,
		})
	}
	...
	var externalResources []shared.PatchV1ServicesServiceIDExternalResources = []shared.PatchV1ServicesServiceIDExternalResources{}
	for _, externalResourcesItem := range r.ExternalResources {
		var remoteID string
		remoteID = externalResourcesItem.RemoteID.ValueString()
		...
		// Service#update.external_resources.removeService#update.external_resources.remove impedance mismatch: boolean != stringtrace=["Service#create.req.external_resources.[].connection_type"]
		var remove1 *bool
		externalResources = append(externalResources, shared.PatchV1ServicesServiceIDExternalResources{
			RemoteID:       remoteID,
			ConnectionType: connectionType,
			Remove:         remove1,
		})
	}
	var functionalities []shared.PatchV1ServicesServiceIDFunctionalities = []shared.PatchV1ServicesServiceIDFunctionalities{}
	for _, functionalitiesItem := range r.Functionalities {
		id1 := new(string)
		if !functionalitiesItem.ID.IsUnknown() && !functionalitiesItem.ID.IsNull() {
			*id1 = functionalitiesItem.ID.ValueString()
		} else {
			id1 = nil
		}
		remove2 := new(bool)
		if !functionalitiesItem.AlertOnAdd.IsUnknown() && !functionalitiesItem.AlertOnAdd.IsNull() {
			*remove2 = functionalitiesItem.AlertOnAdd.ValueBool()
		} else {
			remove2 = nil
		}
		summary := new(string)
		...
		functionalities = append(functionalities, shared.PatchV1ServicesServiceIDFunctionalities{
			ID:      id1,
			Remove:  remove2,
			Summary: summary,
		})
	...
	var links []shared.PatchV1ServicesServiceIDLinks = []shared.PatchV1ServicesServiceIDLinks{}
	for _, linksItem := range r.Links {
		var hrefURL string
		hrefURL = linksItem.HrefURL.ValueString()

		var name string
		name = linksItem.Name.ValueString()
		...
		// Service#update.links.removeService#update.links.remove impedance mismatch: boolean != stringtrace=["Service#create.req.links.[].name"]
		var remove3 *bool
		id2 := new(string)
		if !linksItem.ID.IsUnknown() && !linksItem.ID.IsNull() {
			*id2 = linksItem.ID.ValueString()
		} else {
			id2 = nil
		}
		links = append(links, shared.PatchV1ServicesServiceIDLinks{
			HrefURL: hrefURL,
			Name:    name,
			IconURL: iconURL,
			Remove:  remove3,
	...
	var owner *shared.PatchV1ServicesServiceIDOwner
	if r.Owner != nil {
		var id3 string
		id3 = r.Owner.ID.ValueString()

		owner = &shared.PatchV1ServicesServiceIDOwner{
			ID: id3,
		}
	}
	serviceTier := new(shared.PatchV1ServicesServiceIDServiceTier)
	if !r.ServiceTier.IsUnknown() && !r.ServiceTier.IsNull() {
		*serviceTier = shared.PatchV1ServicesServiceIDServiceTier(r.ServiceTier.ValueInt64())
	} else {
		serviceTier = nil
	}
	var teams []shared.PatchV1ServicesServiceIDTeams = []shared.PatchV1ServicesServiceIDTeams{}
	for _, teamsItem := range r.Teams {
		var id4 string
		id4 = teamsItem.ID.ValueString()

		// Service#update.teams.removeService#update.teams.remove impedance mismatch: boolean != classtrace=["Service#create.resp.teams.[].created_by"]
		var remove4 *bool
		teams = append(teams, shared.PatchV1ServicesServiceIDTeams{
			ID:     id4,
			Remove: remove4,
		})
	}
	out := shared.PatchV1ServicesServiceID{
		AlertOnAdd:            alertOnAdd,
		AutoAddRespondingTeam: autoAddRespondingTeam,
